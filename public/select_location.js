...
function parse_pin_data(brand) {
	...
	for (var elem in pin_data_pairs)
	{
		...
		if (this_data[0][3] == brand) //this_data[idx][3]=brand of the pin
		{
	...
	usersRef.once('value', function(snapshot) {
		...
		if (exists) //check if database is empt
		{
			usersRef.once('value', function(snapshot) {
				...
				window.location.href = "https://cs374-tyty.firebaseapp.com/payment_method.html"; //go to payment method page
			});
		} else {
