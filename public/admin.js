...
function makeTable(list) {
    ...
    for (var i=0; i<list.length; i++) {
      if (list[i].delivered !=2) {
        ...
        cell1.innerHTML = list[i].brand
        cell2.innerHTML = list[i].menu
        cell3.innerHTML = list[i].location
        cell4.innerHTML = list[i].payment
        ...
        if (order_list[i].delivered === 1) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    console.log(data)
                    database.ref("users/"+ changedUid + "/delivered").set(2)
                },
